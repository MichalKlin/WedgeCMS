...
"modarticle,modarticlegrouppage,modarticlegroup",
"modarticlegrouppage_group=modarticle_group and 
modarticlegrouppage_schema=".$SCHEMA_MODULE." 
and modarticlegrouppage_page=".$PAGE."
and activeModArticle='YES' and modarticlegroup_id=modarticlegrouppage_group","
...
if (($ile=$this->baza->size_result($r_art))>0){
	...
	<tr>
	<td class="ramka_group_title">
	<h2>
	<?
	$row_gr = $this->baza->row($r_art);
	echo $row_gr[modarticlegroup_name];
	?>
	</h2>
	</td>
	</tr>
			...
			for ($i=0; $i<$wysw; $i++){
				...
				<tr>
					...
					if (strlen($row_art['contentModArticle'])>0){
						?><!--div class="more"><a href="<?=$site_action?>?a=<?=$row_art[idModArticle]?>&p=<?=$PAGE?>">czytaj więcej...</a></div--><?
					}
					?>
					</td>
				</tr>
	...
	elseif (isset($_GET[a])){
		...
		$row = $this->baza->row($r);
		?>
		<br><center><span class="powrot"><a href="<?=$row[htmlName]?>.html">Przejdź do aktualności</a>
		</span></center><?
