...
$r_as = $baza->select("cmsschema.template as templ","cmsschema,cmsschemamodule","cmsschema.id=cmsschemamodule.schema and cmsschemamodule.id=$_GET[manage]");
$row_as = $baza->row($r_as);
$template_id = $row_as[templ];
//echo $template_id;

$dzis = date("Y-m-d");
$godz = date("H:i");
$r_as = $baza->select("*","modarticlestatictempl","idTemplate=$template_id","","");
$row_as = $baza->row($r_as);

//zapisanie wybranego artukułu
if (isset($_POST['save'])){
	if ($baza->size_result($r_as)==0){	
		$wartsci = "0,$template_id,".$_POST['artykul'];	
		$r = $baza->insert("modarticlestatictempl",$wartsci);	
	}
	else{
		$wartsci = "idArticle=".$_POST['artykul']."";
		$where = "idTemplate=$template_id";
		$r = $baza->update("modarticlestatictempl",$wartsci,$where,"");
	}
}
...
if (isset($_POST['save_article']) and strlen(trim($_POST['title']))>0){			
	...
	";	
	$where = "idModArticle=".$row_as['idArticle'];
...
if (($ile=$baza->size_result($r_art))>0){
	...
	for ($i=0; $i<$ile; $i++){
		...
		if ($row_art['activeModArticle']=='YES'){				
			?>
			<option <?if ($row_as['idArticle']==$row_art['idModArticle']) echo "selected";?> 
	...
	if (strlen($row_as[idArticle])>0){
		$r=$baza->select("*","modarticle","idModArticle=$row_as[idArticle]","order by titleModArticle","");
