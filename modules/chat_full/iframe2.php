...
if (($il = $baza->size_result($r))>0){
	...
	<div style="text-align: left;"><input class="checkbox" type="checkbox" name="refresh" <?if($_SESSION['refresh']=='yes') echo "checked";?> /> auto odświeżanie</div>
	...
	for ($i=0; $i<$il; $i++){
		...
		<tr class="szare<?=$l?>">
			<td>
			<span class="autor"><strong><?=$row2[login]?></strong>&nbsp;[<?=$row[dateModChat]?> <?=$row[timeModChat]?>]
			</span>
			<br>
